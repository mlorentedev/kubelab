...
**Solution**: Fetch `origin/master` and diff it against the branch tip before adding a second unit of work to an existing branch (`git diff origin/master HEAD -- <files>` empty means it's already merged in substance, safe to abandon). When it *is* already merged, branch fresh off `origin/master` for the next piece — carrying uncommitted working-tree changes across a `git checkout -b` is safe and keeps every logically separate addition on its own branch, so a merge decision made on branch A can never eat branch A's future.
