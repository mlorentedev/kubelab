...
- **Verify at GitHub, not by reading the body.** `closingIssuesReferences` is the only authority on what a merge will close. A body that says "No `Closes`" is a claim; the query is the measurement.
