...
**Addendum (2026-08-23, #1357)**: the Solution above describes #227's *intent*, and #227 did not deliver it. `gitea` and `argocd` were registered with each other's values and carried the swap from May until August. Authelia named both faults itself, as mirror images: client `gitea` sent `client_secret_basic` against a registration allowing only `client_secret_post`, and client `argocd` sent `client_secret_post` against a registration allowing only `client_secret_basic`. So the registrations were backwards with respect to what each client actually sends, and the repair was to swap them: `gitea` → `client_secret_basic`, `argocd` → `client_secret_post`.

**The comment was part of the defect, and not by disagreeing with the code.** The `gitea` block carried `# Gitea sends client credentials in POST body … so this MUST be set explicitly`, and the value beneath it *was* `client_secret_post`. Comment and value agreed perfectly; both were wrong about the client. That is the harder shape to catch — [lesson-020](lesson-020-comment-vs-implementation-drift-pair-auto-fil.md) covers a comment that drifts from its code, whereas here nothing drifted, and the confident, specific comment is exactly what stopped anyone re-checking the claim against Gitea. A comment asserting a *third party's* behaviour is an untested claim, however well it matches the line below it; it needs the measurement and its date, which is what replaced it. Fixed in #1357 for the prod overlay (both clients) and for the base that serves staging (`argocd` only — the base declares no `gitea` client). Diagnostic, which needs no inference: Authelia's rejection names both the method it detected on the request and the method the registration allows, so `kubectl logs deploy/authelia -n kubelab | grep invalid_client` identifies any future mismatch and says which side is wrong.
