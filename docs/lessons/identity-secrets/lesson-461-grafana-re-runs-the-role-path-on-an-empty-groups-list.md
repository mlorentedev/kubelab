...
2. if that finds nothing, `{"groups": <groups extracted so far>}`.

The role is extracted before the groups, so on the ID token that second document is
always `{"groups": []}`. The path treated an empty list as "no admin, so Viewer", on
...
per source (`_grafana_role`) instead of evaluating the expression once. A second test
requires every Authelia user to have at least one group. Grafana cannot tell an empty
`groups` from a missing one, so a user in no group would get no role from the path and
keep whatever role it already had.
