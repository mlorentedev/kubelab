...
2. **The chain is order-dependent.** A handler may notify another handler **only if the target is defined after it**. Handlers run in definition order, so a backward `notify` targets something that has already had its turn and is silently dropped — the same invisible failure, with the added trap that moving a handler for tidiness can break it. Two qualifiers matter when generalizing this. The order is not per-file but **play-level**: `ansible-core` collects every handler into one pool for the play, in insertion order — handlers from `roles:`, then the play's own `handlers:`, then `import_role`, then `include_role` — so "defined after" means later *in that pool*, and same-named handlers mask earlier ones. Here both handlers come from the single statically-loaded `beelink_services` role, so file order is the pool order and reasoning locally is sound. And the chain is **conditional on `changed`**: a notifying handler fires its own `notify` only when it reports changed, so a handler that does non-changing work needs `changed_when: true` or the chain breaks exactly like a missing `notify`.
