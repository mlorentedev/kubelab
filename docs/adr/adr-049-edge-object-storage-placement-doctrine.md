...
- **D3 — Storage in 3 tiers; B2 retired.** `tier-offsite` bulk = Hetzner Storage Box + Borg; `tier-offsite` critical subset = R2 (zero egress, free 10GB, reusing the Hermes ADR-004 age+rclone pattern); large S3 artifacts (model weights/datasets for the GPU showcase) = R2-paid or Hetzner Object Storage. **Backblaze B2 leaves the ecosystem**; the B2 portions of ADR-023/ADR-024 are superseded.
  - **Amended 2026-08-15 — the critical-subset mechanism is `restic`, not age+rclone.** This D3 and [#596](https://github.com/mlorentedev/kubelab/issues/596) specified two different tools for the same leg; #596's `restic` is the one that stands, and this line is the record being corrected. The discriminator is verification: restic carries native retention, deduplication and `restic check`, so "prove the backup restores" is a property of the tool rather than a separate mechanism bolted alongside it. The cost is the one this ADR's rationale named — one backup mechanism across the agent + platform fleet — accepted deliberately: Borg already breaks that homogeneity for the bulk tier in this same decision, so the fleet was never going to run a single tool. The destination, the tiering and the retirement of B2 are unchanged. Sequenced in [#1090](https://github.com/mlorentedev/kubelab/issues/1090).
...
- Pattern: Hermes `80_agents/hermes-nan/decisions/004-encrypted-state-backup.md` (age + rclone, prod-proven 2026-06-14) — **superseded as this doctrine's critical-subset mechanism by the 2026-08-15 amendment to D3 (restic).** Still the reference for the agent fleet's own backups.
