---
id: "kubelab-runbook-testing-guide"
type: runbook
status: active
...
- Network tests against live dev/staging/prod environments
- OCP design: `expectations.py` is the single registry, all tests are parametrized over it
- Require: network access, Docker stack running (dev), or VPN (staging/prod)
- Categories: health, content, auth flow, TLS/routing, API validation, security headers, error pages, observability, CrowdSec
- Run: `make test-e2e ENV=dev|staging|prod`

Infrastructure Tests (`@pytest.mark.infra`)
- SSH-based checks against homelab nodes (disk, memory, NTP, Tailscale)
- K3s cluster health (node status, pods, PVCs)
- Network connectivity (VPS reachable, Tailscale mesh, DNS)
- Require: Tailscale VPN connected
- Run: `make test-infra ENV=staging|prod`
