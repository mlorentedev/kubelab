...
The toolkit is a local operator tools that runs on your development machine to manage all environments (dev, staging, prod).
...
```
 ...
 Kubernetes Deployment & Sync (added 2026-03-21)

```bash
make deploy-k8s ENV=staging               Full deploy pipeline: sync images → sync OIDC hashes → apply secrets → kubectl apply
make deploy-k8s ENV=prod                  Same pipeline for production
make sync-k8s-images                      Sync image tags from common.yaml → kustomization.yaml images: transformer
make sync-oidc-hashes ENV=staging         Sync OIDC client_secret hashes from SOPS → Authelia ConfigMap
make sync-oidc-hashes ENV=prod            Same for production
make configure-oidc ENV=staging           Register/update Authelia as OIDC provider in Gitea via CLI
make configure-oidc ENV=prod              Same for production
toolkit secrets hash --env staging        Auto-generate missing OIDC client secrets (plaintext → SOPS, hash → Authelia config)
toolkit secrets hash --env prod           Same for production
```

**Deploy pipeline order** (`make deploy-k8s`):
1. `sync-k8s-images` — reads image versions from `common.yaml`, updates `kustomization.yaml` `images:` entries
2. `sync-oidc-hashes` — extracts argon2 hashes from SOPS, writes to Authelia ConfigMap (hashes are NOT secrets)
3. `apply-secrets` — injects real secret values from SOPS into K8s Secrets (replaces `REPLACE_WITH_SOPS_VALUE` placeholders)
4. `kubectl apply -k` — applies the full Kustomize overlay

**OIDC secret flow:**
- `toolkit secrets hash --env x` generates missing OIDC client secrets: plaintext stored in SOPS, argon2 hash computed
- `make sync-oidc-hashes` copies hashes from SOPS → Authelia ConfigMap (safe — one-way hash)
- `make deploy-k8s` applies secrets (plaintext from SOPS → K8s Secret for Gitea/Grafana env vars)
- Authelia reads hash from ConfigMap, clients read plaintext from K8s Secret

**configure-oidc behavior (updated 2026-03-22):**
- Uses `gitea admin auth update-oauth --id <ID>` (NOT delete + add-oauth)
- Reason: delete fails when users have linked accounts to the auth source
- Script lists existing auth sources first, finds the ID, then updates in place
- If no auth source exists, falls back to `add-oauth` (first-time setup)
- Always restarts Gitea deployment after changes (in-memory cache invalidation)
- OIDC discovery URL MUST be the external Authelia URL (e.g., `https://auth.kubelab.live`) — internal URLs cause issuer mismatch during browser OIDC flows
...
| Deploy K8s (full) | `make deploy-k8s ENV=staging` |
| Sync image tags | `make sync-k8s-images` |
| Sync OIDC hashes | `make sync-oidc-hashes ENV=staging` |
| Configure OIDC | `make configure-oidc ENV=staging` |
| Generate OIDC secrets | `toolkit secrets hash --env staging` |
| Help | `tk --help` |
...
The toolkit is:
-  Local operator tools (runs on your dev machine)
...
Remember: One tools, one location (your dev machine), manages everything everywhere.
