...
ON-DEMAND (homelab, powered when working):
  ...
  Acemagic-2 (12GB)           — Self-hosted developer node / CDE (ace2)
...
- **The Authelia login password is NOT stored and NOT recoverable**: only its argon2id hash exists, at `apps.services.security.authelia.users_<username>_password_hash` per env. There is no `apps.auth.admin_password` — the username alone is SSOT in `common.yaml` (`apps.auth.identities`, currently `{superadmin: manu, operator: operator, machine: hefesto}`) and is plaintext, not a secret. This line named `apps.auth.admin_username` until 2026-08-26; that key was removed on 2026-08-24 by #1390 and `common.yaml:641` records its absence. To change the password: `toolkit credentials hash-password apps.services.security.authelia.users_operator_password_hash --env <env>` then `make apply-secrets ENV=<env>`. **No pod restart needed** — the users file is mounted as a directory with `watch: true`, so Authelia reloads it (the restart gotcha below applies to `configuration.yml`, not the user DB). Do NOT use `make credentials-generate` for this: it regenerates **24 prod secrets and 2 hub secrets** in one shot — every OIDC client secret, plus Grafana, MinIO, Uptime Kuma, basic_auth and the CrowdSec bouncer key. It does **not** touch the HMAC or the session secret (this line claimed it did until 2026-08-23, and the error mattered: describing an operation as more destructive than it is causes paralysis, so an exposed credential goes unrotated). Those two are in `IMMUTABLE_SECRETS` (`toolkit/features/credentials.py:36-41`) with `storage_encryption_key` and `jwt_secret_reset_password`, preserved when they already exist because overwriting them destroys state. **And rotating is not landing**: the command writes SOPS *and* `infra/k8s/overlays/prod/authelia-config/configuration.yml`, then stops. Prod runs `selfHeal: true`, so an uncommitted rotation is reverted by Argo CD to a config that rejects the secrets the cluster was just given — measured 2026-08-23, it took Argo CD SSO down. Commit and merge before, or immediately after, applying. Surgical single-key rotation is #1338. Machine credentials (OIDC client secrets, Grafana/Gitea admin passwords) are reversible in SOPS by design and readable with `secrets-show` — that asymmetry is correct, not an oversight.
- **`make secrets-show` takes the env from `SECRETS_ENV`, NOT `ENV`**: the target hardcodes `ENV=dev` internally and passes `--env $(or $(SECRETS_ENV),common)`. `make secrets-show KEY=x ENV=staging` silently reads `common` and reports "key not found". Correct form: `make secrets-show KEY=x SECRETS_ENV=staging`.
...
- **DNS wildcard covers services NOT on K3s**: `*.staging.kubelab.live` resolves to K3s (100.64.0.4) via CoreDNS wildcard. Services only on Docker Compose have no IngressRoute → Traefik returns self-signed cert. Mark with `skip_in_envs=("staging",)` in expectations.py. **`uptime_kuma` was in that list and does not belong**: it has an IngressRoute in `base/external/uptime-kuma.yaml`, applied in both envs (see the external-services bullet below). The stale entry dates from the 2026-03-19 removal that #121 reverted three days later.
- **Headscale split DNS must target `staging.kubelab.live` only** (NOT `kubelab.live`): Broad split DNS routes ALL `*.kubelab.live` queries to RPi4 — if RPi4 is down, prod domains (which have public Cloudflare records) become unreachable from VPN clients. Narrowing to `staging.kubelab.live` lets prod domains resolve via public DNS (1.1.1.1) regardless of RPi4 state. Changed 2026-03-03. **Headscale `extra_records` is NOT a second path to services outside K3s — it has never resolved, in any state**: `deploy-vps.yml:52` renders each `networking.vpn_extra_records` entry as `<name>.kubelab.live`, a name in the apex zone that the narrowing above deliberately excludes, so the query goes to public DNS instead of MagicDNS. Measured from a workstation on the tailnet with split DNS working: `jetson.kubelab.live` → NXDOMAIN. It is also redundant — MagicDNS already names every node under `kubelab.internal` (`headscale/templates/config.yaml.j2:39`), by its Headscale given-name rather than its `common.yaml` `hostname`: `kubelab-jet1.kubelab.internal` → 100.64.0.8 and `rpi4.kubelab.internal` → 100.64.0.10, both verified. Do not describe it as a bootstrap path; retirement tracked in #964.
...
- **`SecretSpec.envs` is the audit dimension, NOT the storage location**: `envs` declares *which environments must have this secret*, not which file it lives in. `audit(env)` decrypts `common.enc.yaml` and deep-merges `{env}.enc.yaml` over it, so a secret stored in common is resolvable while auditing any env. There is no `common` pseudo-env: every filter is `env in spec.envs`, so registering `envs=("common",)` matches no real env and the secret disappears from *every* audit — silently. A secret that lives in `common.enc.yaml` but is only required by one env is registered with that env (e.g. the dev-node PAT is `envs=("staging",)`; the Argo CD hub keys are `envs=("prod",)`). Note `secrets show`/`set` DO accept `env='common'` — there it selects the file, which is the opposite meaning. (ANSIBLE-033)
...
- **External services through K3s Traefik**: Bare-metal services use Service + EndpointSlice in `infra/k8s/base/external/`. Label: `kubelab.live/location: external`. Current members: Pi-hole, Uptime Kuma. Uptime Kuma's *workload* runs on RPi3 bare metal, but its *routing* is this pattern — `base/external/uptime-kuma.yaml` (Service + EndpointSlice → `100.64.0.6:3001` + IngressRoute, both listed in `base/kustomization.yaml`) — so it is fronted by K3s Traefik, NOT by the Docker Compose Traefik on the VPS (that role is skipped once the VPS is a `k3s_server`). It was removed 2026-03-19 and re-added 2026-03-22 (#121); only the removal note survived. Ollama was the pattern's first user; removed by AI-007 (2026-08-09).
...
- **Two CI gates judge every PR, and one of them can refuse it** (TOOL-021, #1140): `review-attestation` asks only *did a review happen*, from comment **content** rather than from another check's status — because a reviewer reports on itself and a spent quota renders as `success`. Measured: 35 of 40 merged PRs had no review behind a green check. Reviewers are declared in `harness/review-attestation.json`; changing one is a config edit, and no reviewer may be named in `toolkit/features/review_attestation.py` (a test enforces it). **If it refuses your PR, the answers are: get it reviewed, or declare the unreviewed merge with BOTH the `merged-unreviewed` label AND a non-empty `## Unreviewed merge rationale` section** — proceeding unreviewed is allowed, proceeding silently is not. `pr-agent` is the reviewer whose capacity is ours rather than a vendor's, so a quota spent elsewhere stops meaning "no review". **It publishes a commit STATUS, never a check-run**: only the status may go in branch protection, because runs are cancelled by design when a reviewer speaks mid-run and a cancelled job can never conclude green (`gh pr checks` shows `cancelled` in its fail column).
...
- **Apprise `/config` MUST be writable (NOT a read-only Secret mount)**: apprise-api's `/status` healthcheck write-tests `APPRISE_CONFIG_DIR` (`/config`), `APPRISE_ATTACH_DIR`, and `APPRISE_STORAGE_DIR` (`/config/store`); any non-writable dir → `*_PERMISSION_ISSUE` → HTTP 417 → liveness CrashLoops the pod. In `simple` (stateful) mode the SOPS-rendered routing table can't be mounted read-only at `/config`. Pattern: `/config` is a writable `emptyDir` seeded with `kubelab.yml` by an initContainer copying from the `apprise-secrets` Secret (mounted at `/seed`), reusing `caronc/apprise` so no extra image SSOT pin. Verify with `make notify-smoke ENV=staging` (NOTIFY-001).
...
- **Docker containers need explicit `dns:` — but the correct value depends on WHY the host resolver was not inherited**: Docker never passes a host `/etc/resolv.conf` that points at a local resolver into the container, and silently substitutes its own upstreams. Two distinct cases, with opposite fixes — check the container's `# ExtServers:` comment in `/etc/resolv.conf` to tell them apart:
  - **systemd-resolved stub** (`nameserver 127.0.0.53`): the address is the *container's* loopback, not the host's resolver. Any public resolver fixes it — `dns: [1.1.1.1, 8.8.8.8]` (the `docker_dns_servers` default in the `headscale` role).
  - **Tailscale MagicDNS** (`nameserver 100.100.100.100`): Docker falls through to the DHCP/ISP nameservers, which cannot see VPN-only split-DNS zones like `*.staging.kubelab.live`. Here a public resolver is exactly the wrong fix — it resolves prod fine and staging not at all, so the symptom reads as "staging is down" rather than "DNS is wrong". Pin `dns: [100.100.100.100, <public fallback>]` (the `rpi3_services` default). MagicDNS forwards non-split queries onward, so it serves both; the public entry is only there for a Tailscale outage.
  Both roles parameterize this as `docker_dns_servers`. See MON-001 / PR #911.
- **A ufw rule cannot restrict a Docker-published port**: Docker's DNAT is evaluated before ufw's filter chains and `DOCKER-USER` is empty on every node in the fleet, so a published port is governed by its bind address and nothing else — an interface-scoped ufw rule on a published port is decoration. Restrict by binding: `"{{ tailscale_ip }}:{{ port }}:<container_port>"` (the `glances` and `rpi3_services` compose templates), never the bare `"{{ port }}:<container_port>"` form. Corollary from the same investigation: a container reporting `healthy` says nothing about whether its port is reachable — health probes run inside the container, port publishing is kernel state outside it. See #959.
...
- **Loki prod IngressRoute uses `.local` TLD**: ACME can't issue certs for non-public TLDs, so prod must strip the base's `certResolver`. Do this with an explicit `certResolver: null` in `overlays/prod/patches.yaml` — **NOT `tls: {}`**. An empty map in a strategic-merge patch means "change nothing", so the base's resolver survives and Traefik retries an impossible Let's Encrypt order about once a day, forever, in prod. This exact bug shipped and went unnoticed for months because the patch carried a comment saying the resolver was gone. Verify by reading the emitted `tls:` block from `kubectl kustomize infra/k8s/overlays/prod`, never by reading the patch.
- **`make deploy-k8s` applies as Argo CD's service account, not as you** (TOOL-029): the namespaced overlay apply and its dry-run both carry `--as=system:serviceaccount:kubelab:argocd-manager`, read from `infra/k8s/argocd/spoke-rbac.yaml` rather than hardcoded. Reason: your kubeconfig is unrestricted and the spoke's is not, so before this a manifest you could apply was not necessarily one Argo CD could — IDP-031's `LimitRange` passed lint, render, `make test-infra` and a full staging deploy, then was refused in prod (#948), because the GitOps path is only exercised *after* merge. Consequence: if a deploy now fails with `forbidden`, that is the safeguard working — add the resource to the write role in `spoke-rbac.yaml`, do not reach for the escape hatch. `_apply_cluster_bootstrap` is deliberately NOT impersonated (CRDs and cluster-scoped resources need operator privilege). Escape hatch for emergencies: `KUBELAB_DEPLOY_AS_OPERATOR=1 make deploy-k8s ENV=x`, which warns loudly. Belt-and-braces for when the cluster is unreachable: `tests/test_spoke_rbac_covers_manifests.py` asserts statically that every kind in `infra/k8s/{base,overlays}` is writable by that role, so it still runs with the homelab powered off.
...
- **Pi-hole apex name, VPN-reachable only (OPS-022, 2026-08-12)**: `pihole.kubelab.live` — public Cloudflare A record resolving to ace1's Tailscale IP (`networking.nodes.ace1.tailscale_ip`), routed to RPi4 bare metal via LAN EndpointSlice once inside the cluster. Public *resolution* does not mean public *reachability* — the record exists so the name works from anywhere on the tailnet without depending on split-DNS (which itself depends on Pi-hole being up), but the backend is only reachable over VPN/LAN. The old `pihole.staging.kubelab.live` name is retired — `infra/k8s/base/external/pihole.yaml` moved to the staging overlay only, so it never ships to prod (RPi4 is on-demand homelab, never reachable from the always-on VPS). Pi-hole v6 has built-in auth — no Authelia middleware (causes redirect loop). Password in SOPS: `apps.services.network.pihole.admin_password`.
- **Middleware secret injection (ADR-035 Stage 1, AI-001)**: Traefik Middlewares that embed a plaintext SOPS-sourced API key are NOT in Kustomize/git. **`MIDDLEWARE_CATALOG` is currently EMPTY** — AI-007 retired Ollama, Stage 1's only consumer, so `make apply-middleware-secrets` is a no-op until the next service registers (ADR-035 anchors `widget-proxy`/DT-004 and `Pollex public`/AI-004). The `api-key:` plugin stays registered for them. **Because these objects live outside git, Argo never prunes them and no merge removes them — and the API key is inline in the Middleware spec, not in a Secret, so a retired object leaves a plaintext key in etcd until someone deletes it by hand. There is no delete path in the toolkit (TOOL-025).** Toolkit `apply_middleware_secrets()` reads SOPS → renders `infra/k8s/overlays/<env>/middlewares/*.yaml.tpl` → writes gitignored audit copy to `infra/k8s/overlays/<env>/middlewares/.rendered/` → `kubectl apply -f -` via stdin (plaintext never on persistent disk). Triggered by `make apply-middleware-secrets ENV=x` (also auto-runs as part of `make deploy-k8s`). Adding a new auth-protected service requires three SSOT touches: `SECRET_CATALOG` (`secrets_manager.py`), `MIDDLEWARE_CATALOG` (`k8s_middlewares.py`), and a `.tpl` file. Plugin `dtomlinson91/traefik-api-key-middleware` v0.1.2 registered as `api-key:` in `traefik-helmconfig.yaml.j2` (alongside the CrowdSec `bouncer:` plugin). Plugin returns HTTP 403 (not 401) for any rejected request — see plugin README + ADR-035.
...
- **Argo CD selfHeal per environment (ADR-037) — and `selfHeal: false` does NOT mean your staging deploy survives**: `infra/k8s/argocd/applications/staging.yaml` runs with `syncPolicy.automated.selfHeal: false` so staging is a mutable test bed; `applications/prod.yaml` keeps `selfHeal: true` so prod self-corrects drift immediately. Validation flow for any PR touching K8s state: (1) `make deploy-k8s ENV=staging` from worktree, (2) `make apply-secrets ENV=staging`, (3) `kubectl rollout restart deploy/<name> -n kubelab` against staging spoke, (4) `make test-e2e ENV=staging`. Only after green: merge to master, Argo CD prod auto-syncs. **But that window is not guaranteed and closes silently.** Argo CD syncs on two triggers and `selfHeal` governs only one: it suppresses correction of *cluster drift*, and does nothing about the sync `automated` fires whenever *git moves*, which reapplies the tracked manifests wholesale and takes your manual apply with it. So a worktree deploy lives until the next commit lands on master — from **any** parallel lane, a docs-only PR included — and neither the deploy nor Argo CD reports anything. Measured twice at ~30 seconds: lesson-330 (2026-08-15) and #1083 (2026-08-25, during this exact validation flow; **#1083/OPS-023 is open and holds the fix**). Tell it apart from "my change didn't work" with `kubectl get <res> -n kubelab --show-managed-fields` — `kubelab-toolkit Apply` followed by `argocd-controller Update` is this and not you. **Meanwhile the reliable path is to repoint `spec.source.targetRevision` at the feature branch** (lesson-256), which makes Argo CD deploy the branch instead of reverting it — and it must be pointed back at `master` after merge, or staging silently stops receiving master. Corollary: a staging test that shows *no effect at all* is at least as likely to mean "my change is not live" as "my change is broken" — re-verify live state at the moment of measurement, not at deploy time.
...
- **Commit before mutating, when proving a guard goes red.** Mutation testing edits real code and then restores it, and `git checkout -- <file>` is not an undo — it resets to the last *staged* state and its blast radius is the whole file, however small the experiment was. `git add -A && git commit -q -m "wip: ..."` first, after which `git checkout HEAD -- <file>` is safe **by construction** rather than by anyone remembering. This is not hypothetical or a first-time hazard: lesson-344 recorded it on 2026-08-09 and it recurred **three times on 2026-08-22**, twice in the same file, because a written lesson with no mechanism is a reminder and a reminder fails exactly when the situation arrives (lesson-365). Tells that it already happened: a test that passed a minute ago failing on a symbol you just wrote, `git status` calling a file unmodified when you know you changed it, or a failure count exactly equal to a file you just added.
...
- **Service placement principle (ADR-028)**: "Would I need this at 3 AM?" → always-on (VPS/AWS/RPi3). Otherwise → on-demand (homelab). Observability: the **monitoring of record** is prod plus Uptime Kuma on the RPi3 — never staging. The **manifests** (Grafana, Loki, Vector) stay in the shared base and run in both environments, so an observability change is validated in staging before prod (ADR-028 amendment 2026-08-09). Embedding pipeline on VPS (autonomous). CI on Beelink with GitHub-hosted fallback.
...
- **CI runs on GitHub-hosted runners by default (ADR-030, amended 2026-06-26)**: the repo is public, so hosted minutes are free and parallel — `RUNNER_DOCKER` is `["ubuntu-latest"]`. Self-hosted is **opt-in**, kept for any future job that genuinely needs homelab access: `gh variable set RUNNER_DOCKER --body '["self-hosted","linux","docker"]'` (unset falls back to hosted). All workflows route via `fromJSON(vars.RUNNER_DOCKER)`. Fork PRs are forced to `ubuntu-latest` regardless (Docker socket = host access). The Beelink runner (`kubelab-bee`, 4CPU/6GB, tool cache in the `runner_toolcache` volume) still exists but is idle by default.
...
- **ace2 is the on-demand developer node / CDE (ADR-058 D1)**: no hosted services. MinIO and the GH Runner moved to Beelink (ANSIBLE-013 → IDP-024); Ollama was retired outright by AI-007 (2026-08-09). **Its teardown lives in the `dev_node` role**, not in a deleted `ace2_services` role — Ansible is additive, so removing a role never undoes what it installed. Any future service retired from a node needs the same treatment: the cleanup belongs to whatever role still owns the node.
- **Local inference is deferred, not replaced (ADR-029)**: no GPU node exists, so `/v1/llm` stays unbuilt and the interim path is a hosted API. Nothing in `apps/` or `edge/` ever consumed Ollama — AI-007 confirmed zero runtime consumers against ~36 documents describing it, which is the shape to expect when retiring a capability that was decided but never wired in.
