...
- **RESOLVED — the replaced node's address is a name, not an IP.** `ansible_host` for aws1 is `aws1.kubelab.internal`, a MagicDNS name (`common.yaml:329`, ADR-025), and the generator reads `tailscale_dns` in preference to `tailscale_ip` (`generator_ansible.py:136`). This is the whole point of ADR-025 — aws1 is cattle, and ADR-047 records the same reasoning for RPi4, whose "Tailscale IP rotates on re-registration". Verified live: `dig +short aws1.kubelab.internal` answers `100.64.0.7` *while the instance is stopped*, so the record survives a Spot interruption. No inventory edit is needed after a replacement.
- **OPEN, and it is the load-bearing dependency of the bullet above — MagicDNS resolves the *given-name*, not the `hostname` field.** `common.yaml:585` states this explicitly for a different consumer: *"Headscale names nodes by their given-name, which is not guaranteed to equal the `hostname` field."* CLAUDE.md records the fleet actually diverging — `kubelab-jet1.kubelab.internal`, not `jet1`. So `aws1.kubelab.internal` resolving today is a *fact about the current registration*, not a property of replacement: re-registration without stale-node cleanup previously yielded `aws1-<random>` (playbook header, lesson 2026-03-26), and that name would break both the inventory and the hub kubeconfig.

  The vault's NET-001 makes the dependency concrete: MagicDNS naming in this fleet was established with an explicit `headscale nodes rename` as its step 1, and aws1 is still the only node using a name rather than an IP. So `aws1` as a given-name is a *maintained* property, not an emergent one.

  cloud-init performs the stale-node cleanup via the Headscale API key, so the mechanism to keep it maintained exists — but this spec must not treat it as a closed risk the way an earlier draft of this section did. Task breakdown must assert the resolved name after replacement rather than assume it, and the wait in AC2 should fail on an unresolvable name with a diagnostic naming *this* cause, since "host unreachable" is the least informative way it can present.
