...
- [x] T4: `toolkit services gitea pr create` / `issue create`, with a CLI test that the password never
      reaches output.
- [x] T5: live: open the `fix/onboarding-figure` PR and the worktree-defect issue on `personal/resume`.
- [x] T6: replace "Manu opens the PR in the web UI" in `resume`'s memory with the command.
