...
The watcher reads R2 on its existing 6h schedule, from inside the cluster, with a **read-only** R2 token and the restic repository password. For every node in `backup.sources` it checks three things, and nothing else:
...
- [ ] AC3: mutation proof in staging. Each of the following turns the fleet line to `healthy:0` and fires the rule, then reverts:
  - a declared source that does not exist (an added fake service in the generated targets file);
  - a node with no repository;
  - a wrong restic password.
