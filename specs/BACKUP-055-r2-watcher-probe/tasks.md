...
2. [x] (✓ 2026-09-26, `bc5996dd`) [P] [AC4] Test first, `tests/test_r2_watcher_targets.py`: the committed `infra/k8s/base/services/r2-backup-watcher/targets.txt` equals the render from `backup.sources` plus `repository_name()`, one `<node> <repository> <service>...` line per node. Watch it fail.
3. [x] (✓ 2026-09-26, `bc5996dd`; the `configMapGenerator` wiring moves to task 7, where the CronJob that mounts it is rewritten) [AC4] Add the renderer (`toolkit/features/backup_destination.py`, `watcher_targets()`) and `toolkit sync r2-watcher-targets`, generate the file, and serve it through `configMapGenerator` `files:`. The hash is kept: each Job pod reads it fresh, but a hash is never wrong. Green.
4. [x] (✓ 2026-09-26, `05034bb5`) [P] [AC5] Test first: `backup.watcher_image` is in `IMAGE_SOURCES`, its tag equals `backup.r2.restic_version`, and `kustomization.yaml` `images:` carries it. Then add the SSOT key and run `make sync-k8s-images`. Green.
5. [x] (✓ 2026-09-26, `3294d9b9`) [P] [AC1] Catalog `backup.r2.readonly_access_key_id` and `backup.r2.readonly_secret_access_key`: `SecretKind.EXTERNAL`, `envs=("staging","prod")`, stored in common. Add a `SecretMapping` `r2-backup-watcher-secrets` holding `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and `RESTIC_PASSWORD`. Test the mapping, following `test_k8s_secrets_vikunja_r2.py`.
6. [x] (✓ 2026-09-26, `2c832216`) [AC1] [AC3] Test first, `tests/test_r2_backup_watcher_probe.py`: run the probe script under `sh`, with a fake `restic` on PATH, to prove:
