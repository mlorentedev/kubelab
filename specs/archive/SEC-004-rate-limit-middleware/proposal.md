...
- **Two Traefik RateLimit semantics to pin explicitly during implementation, not assume:**
  - **`sourceCriterion`**: with klipper-lb masquerading every client to an internal pod-CIDR address (#1067), Traefik's *default* client-IP keying would accidentally collapse everyone into a near-global bucket anyway — but that's "global by accident," not by design. Set `sourceCriterion.requestHost: true` explicitly (Traefik's CRD schema expects a boolean here, not an object — confirmed via a rejected dry-run apply during implementation) so the behavior doesn't silently change the moment #1067 is fixed and real client IPs reappear.
  - **[Resolved 2026-08-15, measured] Shared vs. per-router bucket: confirmed per-router, independent buckets.** Burst drill in staging: exhausted `home.staging.kubelab.live`'s bucket (300 requests, 88 got 429), then immediately probed `grafana.staging.kubelab.live` — 5/5 clean (302, its normal unauthenticated redirect), proving no shared state. Bursting grafana directly then also produced 429s (104/300) under its own load, confirming it has an independent bucket rather than being exempt. This is the better outcome for a blanket-applied limit: a shared bucket would mean one route's burst starves every other route's legitimate traffic — a rate-limiter-induced cross-route DoS, which independent buckets avoid. **Known limitation, not a gap**: because the keying is `requestHost`, the catch-all route's `HostRegexp` match gets a fresh bucket per distinct hostname a client sends — a Host-cycling scanner spraying random subdomains sidesteps that route's limit. Acceptable here (catch-all's backend is the static `errors` service; CrowdSec's reputation-based bouncer, not this rate limiter, is the layer meant to handle that kind of abuse), but worth recording rather than leaving for a future reviewer to rediscover.

## Acceptance criteria

- [x] A `rate-limit` Middleware object exists (mirrors `secure-headers.yaml`'s pattern), values sourced from `edge.traefik.rate_limit_k3s_*` in `common.yaml` (a new, K3s-specific SSOT key — see Risks above), no hardcoded duplicate. ✓ 2026-08-15
- [x] Every K3s IngressRoute, blanket — public and VPN-only alike (base + generated `ingress.yaml` for both envs) — includes `rate-limit` in its middlewares list, positioned consistently with the existing `secure-headers`/`error-pages`/optional-auth chain. ✓ 2026-08-15
- [x] A synthetic burst test (staging) proves the limit fires above threshold (HTTP 429) and passes clean below it — exercised, not assumed, matching this session's established verification discipline (OBS-010's `max_over_time`→`last_over_time` catch was found exactly this way). The clean-below-threshold case must not inherit token-bucket state consumed by the burst case: either wait for the bucket to refill (per the configured `average`/`period`) between the two runs, or exercise them against an isolated limiter fixture, so a false 429 from bucket exhaustion can't be mistaken for a real failure. The same drill also settles whether the bucket is shared across routers or per-router (exhaust route A, immediately probe route B) — record which, since it changes what "blanket application" actually guarantees. ✓ 2026-08-15 — see verification.md for the full numbers.
- [x] `make config-check-drift` stays green after the `_build_middlewares()` change — regenerated `ingress.yaml` for both envs is part of the same commit as the generator change. ✓ 2026-08-15
