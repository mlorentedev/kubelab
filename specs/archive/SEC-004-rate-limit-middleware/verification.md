...
- **Prod verification (2026-08-15, post-merge — Part 5):**
  - `make check-apps`: `kubelab-prod` Synced/Healthy at the merge revision; no Application drift
  - `make test-infra ENV=prod`: **40 passed, 10 skipped, 0 failures** — the whole prod infra suite, including all 5 of `tests/infra/test_rate_limit.py`
  - The prod overlay renders **17** routes after #1089, not the 16 recorded above: the `argo.kubelab.live` route joined the overlay in that PR. All 17 carry `rate-limit`.
...
- **Part 5 found a route the spec's own instruction would have missed, and the static test could not see.** The task said `make deploy-k8s ENV=prod`; that target does not touch `overlays/prod/argocd.yaml`, which sat outside the Kustomize overlay because *one* of its three documents carries a `RESOLVE_*` placeholder. So `argo.kubelab.live` ran unlimited while `kubelab-prod` reported `Synced` — correctly, since Argo CD does not manage what is not in the overlay. Verified by reading the live cluster, not the manifests: the file was right the whole time. Fixed in #1089 by splitting the manifest (only the EndpointSlice stays out-of-band, matching `headscale.yaml`'s already-committed treatment), after which the route arrived via GitOps with no manual prod deploy. The correction to the framing matters as much as the fix: this was first mis-diagnosed as implementation drift from ADR-047 D3, until `specs/TOOL-009-.../tasks.md` T4 turned up recording the split as a deliberate decision — the proposed remedy at that point (adding `envs:` to `cluster_bootstrap`) would have contradicted it.
...
- [x] Lesson for the repo's `docs/lessons.md`? Yes — three: (1) already captured pre-implementation: "K3s's built-in ServiceLB masks every client's real IP — `externalTrafficPolicy: Local` cannot fix it" (2026-08-14, tags `#k3s #networking #servicelb #klipper-lb #sec-004 #gotcha`). (2) staging's Argo CD auto-syncs on any new master revision even with `selfHeal: false`, which can silently revert an in-progress manual staging deploy on a multi-lane day — see #1083 for the diagnosed mechanism. (3) New, from Part 5: one placeholder document exiled an entire three-document manifest from GitOps, and `Synced` reported nothing wrong because a Synced Application only speaks for the resources it manages — written this session, enforced by `tests/test_orphan_manifests.py`.
