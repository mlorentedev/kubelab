...
**Pre-flight added during implementation (not in the original plan, and it can veto the phase).** A `LimitRange` is not purely additive: `default` is injected as a limit into any container that omits one, and `request <= limit` is validated afterwards. So a container declaring `requests.memory` above 256Mi with no limit would be *rejected* — the object meant to protect the namespace breaking pods. Measured across both clusters before the manifest was written: 0 containers in that category, 35 safe, only the 2 known unbounded initContainers. Had that set been non-empty, the chosen values would have needed revisiting first.

> **Corrected 2026-08-17 (IDP-035, #1130).** This paragraph originally claimed a mirror case — a declared limit *below* the 128Mi `defaultRequest` — and reported it as measured at zero. **That case cannot occur**, so measuring it proved nothing. For a container with a limit and no request, the API server's own pod defaulting sets `request = limit` at decode time, *before* LimitRanger runs, so `defaultRequest` is never applied to it. Verified by server-side dry run against both live clusters: `limits.memory: 64Mi` is ADMITTED with `requests.memory: 64Mi`, not rejected.
>
> The risk that does exist was never named: such a container is charged against the quota's `requests.memory` at its **full limit value**, not at 128Mi. Harmless at the values in force and with zero containers in that shape today, but anyone revisiting these numbers against the original text would be reasoning from a mechanism Kubernetes does not have.
