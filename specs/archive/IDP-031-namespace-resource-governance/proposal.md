...
- [ ] A `ResourceQuota` constrains `requests.memory` at 4 Gi and `limits.memory` at 7 Gi, the same values in both environments, and the namespace reports usage against them. *(Wording corrected 2026-08-17, IDP-035: this said `kubectl describe ns kubelab` reports it. The test reads `status.hard` / `status.used` from the object instead — deliberately, per the `tls: {}` lesson: never assert on a rendering. The JSON check is strictly stronger, so the wording moved to the criterion rather than the check moving to the wording.)*
