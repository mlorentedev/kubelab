...
- [ ] [AC6][AC7] **Restrict the slash-command path to OWNER/MEMBER/COLLABORATOR.** Not in the upstream design because upstream's repo is PRIVATE and this one is public: any account can comment on a public PR, `issue_comment` runs in base-repo context holding `NAN_API_KEY`, and the unrestricted form lets a stranger spend the inference budget and exercise `pull-requests: write`. Same association set the attestation classifier uses, so "belongs to this repository" has one definition.
