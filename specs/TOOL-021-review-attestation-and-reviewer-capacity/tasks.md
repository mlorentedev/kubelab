...
- [ ] [AC8] **Skip release PRs at the JOB level**, `if: !startsWith(github.head_ref, 'release-please--')`. This is AC8's *reviewer* half and it had no task until review caught it — the criterion says a release PR is **neither reviewed nor left pending**, and only the gate half was covered. A marker-based cross-check cannot see a task that covers half its criterion, which is this spec's own audit-scope defect turned on itself. Branch-name matching is wrong for the gate (gaming it bypasses review) and right here (gaming it forfeits the review, and the gate then goes red).
- [ ] [AC6] **Include `[pr_code_suggestions]`** — `dual_publishing_score_threshold`, `suggestions_score_threshold`, `publish_output_no_suggestions`. Omitting it is how upstream shipped this tool with its stated purpose unconfigured: inline publishing defaults to **disabled** (`-1`), and they measured 2 bot comments and **0 inline** across three PRs before noticing.
- [ ] [AC6][AC7] **Restrict the slash-command path to OWNER/MEMBER/COLLABORATOR.** Not in the upstream design, and the reason is NOT that upstream is private — measured 2026-08-19, it is public too, so the unrestricted form was a live exposure there rather than a contextual difference (reported, and fixed on their side). Any account can comment on a public PR, `issue_comment` runs in base-repo context holding `NAN_API_KEY`, and the unrestricted form lets a stranger spend the inference budget and exercise `pull-requests: write`. Same association set the attestation classifier uses, so "belongs to this repository" has one definition.
- [ ] **Assert the two-file name agreement in a test.** The gate re-evaluates on `workflow_run: workflows: [pr-agent]`, so the reviewer workflow's `name:` is a contract. Rename either side and re-evaluation stops firing **silently** — the reviewer reviews, the gate never looks again, and every PR it reviewed reads unreviewed.
- [ ] **Drop `model_weak` rather than port it.** `auto_describe` is off so it would do nothing, and the model upstream names is one `harness/reviewer-pool.json` rejects on quality grounds here. Importing a documented tension for zero behaviour is the worst of both.
