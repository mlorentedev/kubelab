...
- **Retiring CodeRabbit or Codex.** Deliberately deferred to a decision with data from the parallel window. Recorded here so the window has a declared end rather than becoming permanent by omission. **Resolved 2026-08-19 — the count was the wrong question.** This bullet asked whether a third reviewer breaches an unlocatable cap of two. It does not, because the three are not equivalent. CodeRabbit and Codex are **opportunistic**: they contribute when their account quota allows and both were measured exhausted at the same moment. PR-Agent is the **floor**: its capacity is ours, so a spent quota elsewhere stops meaning "no review". A cap on *how many reviewers* answers a question nobody has; what matters is that at least one is guaranteed and that its own failure is visible — which is `fallback_models = []` plus Part 1. Retiring an opportunistic reviewer stays out of scope and is now a preference rather than a constraint. **The window's end condition is deliberately unset, and does not gate this spec.** "What observation retires a reviewer" is a question about the retirement, which is what this bullet declares out of scope — answering it now would be guessing from zero comparative data. It is the first question the follow-up ticket asks, not a precondition for starting. **Open decision, surfaced not settled:** the operator chose "alongside, bounded window" while the count was believed to be one existing reviewer. It is two, so Part 2 would make three. dotfiles cites a standing rule capping autonomous reviewers at two — in a runbook, a proposal and a workflow comment — but the rule itself could not be located in `AGENTS.md` or the harness, which is the same shape as a guard citing a check it cannot run. Whether the cap binds kubelab, and if so which reviewer PR-Agent displaces, is the operator's call before Part 2 starts.
...
- **This spec is machinery about machinery, and kubelab's backlog says that is the expensive kind of work.** dotfiles amended its incident-to-guard rule tonight precisely against this shape: guards, specs and review machinery get the cheapest honest fix, deletion is preferred to construction, and meta work is capped while opened exceeds closed. Its trigger was 191 open issues at +18/week. **kubelab is at 438 open, 54 opened against 34 closed in the last 7 days — +20/week net, on more than double the backlog.** kubelab does not carry that rule (grepped `AGENTS.md`, `CLAUDE.md` and the global harness; absent), so nothing here is in breach. But the argument that produced it applies with more force here, and the honest consequence is that **Part 1 and Part 2 should be weighed separately**: Part 1 makes an existing failure visible and is the cheapest fix that ends it, while Part 2 is construction — a third reviewer, against a cap nobody can locate, to remove a cause Part 1 already renders harmless. **Recommendation withdrawn on measurement.** An earlier draft of this bullet recommended shipping Part 1 alone. That is wrong, and the reason is specific to this repo: **no human other than the PR author can ever attest here.** The last 20 merged PRs are 20 of 20 authored by `mlorentedev`, no PR has ever carried a review from another human, and attestation deliberately excludes the PR's own author. So the only possible attesters are bots — and Part 1 alone turns "both bots are out of quota" from a silent green into a **hard block with no manual remedy**. That is not making a failure visible; it is a deadlock in a single-maintainer repo. **Part 2 is what makes Part 1 survivable**, so they ship together and Part 1 must not be promoted to required until a reviewer without an account-wide quota is running. The backlog argument above still stands as a reason to keep the scope exactly this and add nothing to it.
