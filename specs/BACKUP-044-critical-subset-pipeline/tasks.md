...
- [ ] [AC1] Add the `pvc` source type and the VPS's remaining two consumers (`kubelab/authelia-data`, `kubelab/postgres-data`). **Deliberately deferred, not forgotten**: restoring a PVC differs in kind — the claim must exist before anything can be written into it — so the design belongs with the restore work in Part 6 rather than with a declaration. Both currently have an in-cluster copy via the PVC CronJob, which is not offsite; that is the gap this closes. Until it lands, the CronJob keeps running and MinIO keeps a consumer, so **this spec does not release #972 even when complete**.
