...
- [ ] On `release_created` for `api`/`errors`, **no `docker build`** runs for the semver tag — the semver image is produced solely by re-tagging an existing `sha-<short>` digest.
- [ ] `kubelab-<app>:X.Y.Z` resolves to the **same manifest-list digest** as the staging-validated `kubelab-<app>:sha-<short>` (identical amd64 + arm64).
