...
- **R4 — a bot account must not be able to log in through SSO. ANSWERED 2026-08-23 — `prohibit_login` via the admin API, not the CLI; see `verification.md`.** Original text follows — Decided in principle by ADR-062 D1 (machine identities hold a scoped token and nothing else); what remains is which mechanism enforces it — `prohibit_login` on the Gitea account, never declaring it in Authelia, or both. Enforced, not conventional.
