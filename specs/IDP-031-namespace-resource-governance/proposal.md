...
- **Resolved, and it settles the numbers: #917.** ADR-028 decided that observability runs on prod and not staging, with a circularity argument, while staging runs Grafana, Loki and Vector anyway — so the "identical workloads" premise was measuring drift rather than intent. Resolved toward amending the ADR in **#920 (open, pending merge)**: the ADR's bullet conflated *where the monitoring of record lives* (prod plus the RPi3 — unchanged) with *which manifest set each environment deploys* (staging mirrors prod's base, per the ADR-037 validation flow). Observability manifests stay shared. The identical footprints are therefore a deliberate architectural property, and a single ceiling applied to both environments is the design that follows from it. **This spec should not be frozen until #920 merges**, since the premise it now cites lives there.
