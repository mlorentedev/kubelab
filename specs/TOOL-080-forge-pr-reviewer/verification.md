...
- 2026-09-26, Manu: **the PR-Agent webhook secret moves into PR 3**, closing the open decision PR 3 first raised (a missing secret would have made `execute` refuse the hook on every declared repository until PR 4 landed). `SECRET_CATALOG` gained `apps.services.automation.pr_agent.webhook_secret` (RANDOM_HEX, length 32, prod only), modelled on n8n's `forge_webhook_secret`, next to it in a new "PR-Agent" section. `nan_api_key` — the OTHER secret PR 4's task item named — stays in PR 4: it is EXTERNAL, copied from Bitwarden, and moving it needs no reconciler change, unlike this one, which the reconciler was already refusing to write without.
  - Value generated as `secrets.token_hex(32)` and piped over stdin into `toolkit secrets set --env prod --stdin`, never printed, decrypted to stdout, or passed through argv. The `prod.enc.yaml` diff adds only `pr_agent.webhook_secret`, plus SOPS's `mac` and `lastmodified`. A first attempt with `toolkit secrets init --env prod --rotate <key>` re-encrypted every value in the file (130 changed lines). That diff cannot be reviewed and conflicts with any other SOPS change, so it was discarded before push.
