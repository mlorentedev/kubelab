...
- PR 3 (webhook list), 2026-09-26: `poetry run pytest -q` → 2738 passed, 15 skipped, 156 deselected, 1 xfailed. `make lint` and `poetry run mypy toolkit` both clean; `poetry run toolkit sync all --check` passes (`platform.json` regenerated once, over its `source_commit` hash of `common.yaml`).
  - `plan_reconcile`/`execute` moved from one `declared_webhook: WebhookSpec` to `declared_webhooks: Sequence[HookDeclaration]`; every existing single-hook test in `test_gitea_repo_reconcile.py`, `test_gitea_repo_execute.py` and `test_gitea_review_team.py` was mechanically migrated to a one-hook tuple (`N8N_HOOK_ONLY` / `DECLARED_WEBHOOKS` in the execute file) and re-ran green, unchanged in what it asserts.
...
- **Open, from PR 3 (2026-09-26): the PR-Agent hook has no secret in SOPS yet, and `execute` was left refusing to write it rather than given a skip.** `apps.services.automation.pr_agent.webhook_secret` is PR 4's scope (tasks.md), not this one's, and PR 3 declares the hook `active: false` for exactly the reason stated in tasks.md — "so no delivery goes to a host that does not answer yet." But `execute`'s existing refusal (missing secret -> recorded failure, hook not written) does not know or care about `active`; it refuses this hook on every `--apply` between PR 3 merging and PR 4 adding the key, on every declared repository, and `report.ok` is False for that reason alone even when every other declared change lands. Two ways to close it, neither taken here: (a) PR 4 lands the SOPS key and catalog entry first, so the window is real but short; (b) a caller-side skip in `services.py` that omits an `active: false` hook with no resolvable secret from the plan entirely — not implemented, because it is a policy decision (does a plan-only run also skip it and under-report the work, or only `--apply`?) that the proposal does not make and this PR should not invent. `plan_reconcile` itself cannot skip: it is pure and never sees a secret value. Recorded here rather than silently resolved either way; Manu decides which.
